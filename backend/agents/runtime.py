...
def _qa_mode_prompt() -> str:
    return (
        ...
        "HANDLING QUESTIONS:\n"
        "  • EXPLAIN THIS SLIDE: 'explain this', 'explain this slide', 'what's on this slide',\n"
        "    'explain a bit', 'walk me through this' → call get_slide_details for the CURRENT\n"
        "    page (omit page_number or pass CURRENTLY SHOWING). Narrate from slide_content.\n"
        "    Do NOT ask what to explain. Do NOT call fetch_external_data first.\n"
        "  • For other content questions → call search_and_answer first. Never answer\n"
        ...
        "  • CLARIFICATION: Only if the user is vague AFTER you already gave a substantive\n"
        "    answer ('tell me more', 'elaborate' with no topic), ask what to dive into and\n"
        "    offer 2-3 topics. First-time 'explain this' is NOT clarification — narrate the slide.\n"
        "  • For off-topic questions (clearly outside this presentation AND not on the\n"
        "    current slide) → use fetch_external_data. Do not apologize or refuse.\n\n"
        "FOLLOW-UP:\n"
...
def _tool_rules() -> str:
    return (
        ...
        "  RULE 5 — EXPLAIN / VISUAL / CURRENT-SLIDE QUESTIONS:\n"
        "    'explain this', 'explain this slide', 'what's on this slide', 'explain a bit',\n"
        "    'What does this chart show?', 'explain this graph' → call get_slide_details\n"
        "    for the current page instead of search_and_answer. Narrate; do not clarify first.\n"
        "  RULE 6 — EXTERNAL / BRIEFCASE ONLY WHEN MISSING ON-SLIDE:\n"
        "    Suggest Briefcase or fetch_external_data only when on-slide content does not\n"
        "    cover the asked topic. If the visible slide mentions the topic (e.g. 'Problem\n"
        "    Matching'), answer from the slide and stay put.\n"
        ...
        "get_slide_details:\n"
        "  • Use for 'explain this / this slide', 'this chart', 'this graph', or any\n"
        "    visual/structural element of the currently visible slide.\n"
        "  • Prefer omitting page_number (defaults to current page) or pass CURRENTLY SHOWING.\n"
        "  • Narrate from returned slide_content; never claim the slide is empty if content exists.\n\n"
        "fetch_external_data:\n"
        "  • Use for data NOT on the slides (historical performance, external stats, etc.).\n"
        "  • Do NOT use when the current slide already answers the question.\n"
