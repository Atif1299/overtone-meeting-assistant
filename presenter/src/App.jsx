...
export default function App() {
  const { session, presentation, wss } = parseQuery();
    ...
    fetch(`${base}/api/v1/presentations/${presentation}`, {
      ...
      .then((data) => {
        ...
        if (pages > 0) {
          setTotalPages(pages);
          setUiError("");
          setStatus((prev) => (prev === "error" ? "listening" : prev));
        } else {
          setUiError("Presentation metadata missing total_pages");
        }
      })
      .catch((err) => {
        if (cancelled) return;
        console.error("Failed to load presentation metadata", err);
        setUiError("Failed to load presentation — check API base URL");
        setStatus("error");
      });
    return () => {
      cancelled = true;
  ...
  const { connected } = usePresentationTransport({
    ...
    onError: useCallback((err) => {
      console.error("Presentation transport error", err);
      setUiError(String(err?.message || err || "Transport error"));
      setStatus("error");
    }, []),
  });
  ...
  const { realtimeStatus } = useRealtimeAgent({
    ...
    onError: useCallback((err) => {
      console.error("Realtime agent error", err);
      setUiError(String(err?.message || err || "Realtime connection error"));
      setStatus("error");
    }, []),
  });

  const indicatorStatus = useMemo(() => {
    if (uiError || status === "error") return "error";
  ...
  }, [connected, isRealtimeMode, realtimeStatus, status, uiError]);
